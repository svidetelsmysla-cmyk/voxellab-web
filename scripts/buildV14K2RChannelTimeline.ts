...
const rootsJsonlPath = optionalArgument("--roots-jsonl");
if (!outputPath && !rootsJsonlPath) throw new Error("missing --output or --roots-jsonl");
...
const rootsFd = rootsJsonlPath ? openSync(rootsJsonlPath, "w") : undefined;
if (rootsFd !== undefined) writeSync(rootsFd, `${JSON.stringify({
  schema: "V14K2R_RAW_CHANNEL_ROOT_FRAMES_JSONL_V1",
  packet_id: packet.packet_id,
  channel_id: channelId,
  frame_count: packet.frames.length,
...
try {
  for (let frameIndex = 0; frameIndex < packet.frames.length; frameIndex++) {
    ...
    const roots = findQualifiedRoots(
      volume,
      frameIndex,
      2e-4,
      { ...DEFAULT_THRESHOLDS, neutralTolerance },
    );
    if (rootsFd === undefined) {
      neutralThresholds.push(neutralTolerance);
      rootsByFrame.push(roots);
    } else {
      writeSync(rootsFd, `${JSON.stringify({ frame_index: frameIndex, neutral_threshold: neutralTolerance, roots })}\n`);
    }
    console.log(`roots channel=${channelId} frame=${frameIndex + 1}/${packet.frames.length}`);
  }
} finally {
  if (rootsFd !== undefined) closeSync(rootsFd);
}
if (rootsFd !== undefined) {
  console.log(JSON.stringify({ status: "V14K2R_RAW_CHANNEL_ROOT_FRAMES_BUILT", channel_id: channelId }, null, 2));
} else {
